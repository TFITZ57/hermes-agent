...
def _build_gbrain_span_context(task_id: str = "") -> dict[str, str]:
    """Return per-call env vars that let gbrain emit canonical Opik spans."""
    try:
        from gateway.session_context import get_session_env
    except Exception:
        get_session_env = lambda name, default="": os.getenv(name, default)  # noqa: E731
    ...
    context = {
        ...
        "sessionKey": get_session_env("HERMES_SESSION_KEY", os.getenv("HERMES_SESSION_KEY", "")),
    }
    clean = {key: value for key, value in context.items() if isinstance(value, str) and value}
    return {"HERMES_GBRAIN_SPAN_CONTEXT": json.dumps(clean, ensure_ascii=False)}


@contextmanager
def _temporary_env_overrides(env_obj, overrides: dict[str, str]):
    """Temporarily apply per-call env vars to an environment object."""
    env_map = getattr(env_obj, "env", None)
    if not isinstance(env_map, dict) or not overrides:
        yield
        return

    sentinel = object()
    previous = {key: env_map.get(key, sentinel) for key in overrides}
...
def terminal_tool(
    ...
    try:
        ...
        _gbrain_span_env = _build_gbrain_span_context(effective_task_id)
        ...
        if background:
            ...
            try:
                with _temporary_env_overrides(env, _gbrain_span_env):
                    if env_type == "local":
